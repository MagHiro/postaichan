...
export async function PATCH(request: Request, { params }: Context) {
  ...
  try {
    ...
    return NextResponse.json({ error: duplicate ? "Kode meja sudah digunakan." : "Perubahan meja belum tersimpan." }, { status: duplicate ? 409 : 503, headers: noStoreHeaders() });
  }
...
export async function POST(request: Request, { params }: Context) {
  ...
  try {
    ...
    return NextResponse.json({ error: "QR meja belum dapat dirotasi." }, { status: 503, headers: noStoreHeaders() });
  }
  const data = result.rows[0];
  if (!data) return NextResponse.json({ error: "Meja tidak ditemukan." }, { status: 404, headers: noStoreHeaders() });
