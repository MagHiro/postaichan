...
export async function POST(request: Request) {
  ...
  try {
    ...
    const { error: eventError } = await supabase.from("payment_events").insert({ provider_event_key: providerEventKey, provider_transaction_id: event.transaction_id, event_type: event.transaction_status, payload: event, verified: true });
    ...
    if (settled) await supabase.from("orders").update({ status: "paid" }).eq("id", payment.order_id).in("status", ["awaiting_payment", "draft"]);
