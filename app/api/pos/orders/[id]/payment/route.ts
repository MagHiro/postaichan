  ...
  try {
    ...
    if (synchronizeProvider && status === "pending" && payment.provider === "midtrans" && payment.provider_order_id) {
      try {
        ...
        // Keep the stored state during provider outages, but tell the operator
        // why synchronization could not complete.
        ...
        return NextResponse.json({ ...failure, paymentStatus: status }, { status: failure.status, headers: noStoreHeaders() });
      }
    }
