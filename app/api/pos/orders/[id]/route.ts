...
type OrderItemDetail = { id: string; product_name_snapshot: string; quantity: number; unit_price_idr: number; line_total_idr: number; note: string | null; order_item_modifiers: Array<{ modifier_type: string; modifier_name_snapshot: string; price_adjustment_idr: number }> };
...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    for (const row of itemsResult.rows) {
      const item: OrderItemDetail = itemMap.get(row.id) ?? { id: row.id, product_name_snapshot: row.product_name_snapshot, quantity: row.quantity, unit_price_idr: row.unit_price_idr, line_total_idr: row.line_total_idr, note: row.note, order_item_modifiers: [] };
