...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (changed.rows[0]?.transition_order_status !== true) return NextResponse.json({ error: "Pesanan sudah berubah atau tidak dapat dipindahkan ke status itu." }, { status: 409, headers: noStoreHeaders() });
    ...
    return NextResponse.json({ error: "Status pesanan belum dapat diperbarui." }, { status: 503, headers: noStoreHeaders() });
  }
}
