...
async function getPaymentStatus(request: Request, { params }: Context, synchronizeProvider: boolean) {
  ...
  try {
    ...
    if (!(await consumeRateLimit(request, "payment-status", 30, 300, sessionHash.slice(0, 24)))) return NextResponse.json({ code: "PAYMENT_STATUS_RATE_LIMITED", error: "Polling terlalu sering. Tunggu sebentar sebelum mengecek lagi." }, { status: 429, headers: { ...noStoreHeaders(), "Retry-After": "60" } });
    ...
    if (!session) return NextResponse.json({ code: "SESSION_EXPIRED", error: "Sesi pemesanan sudah berakhir. Scan QR terbaru." }, { status: 401, headers: noStoreHeaders() });
    ...
    if (sourceTableId && (await query("select id from public.restaurant_tables where id = $1 and active = true and qr_token_version = $2", [sourceTableId, sourceTableVersion])).rowCount === 0) return NextResponse.json({ code: "TABLE_QR_NOT_AVAILABLE", error: "QR meja sudah tidak berlaku. Scan QR terbaru." }, { status: 410, headers: noStoreHeaders() });
    if (session.ordering_qr_code_id && (await query("select id from public.ordering_qr_codes where id = $1 and active = true and token_version = $2", [session.ordering_qr_code_id, session.ordering_qr_token_version])).rowCount === 0) return NextResponse.json({ code: "QR_NOT_AVAILABLE", error: "QR pemesanan sudah tidak berlaku. Scan QR terbaru." }, { status: 410, headers: noStoreHeaders() });
    ...
    if (!order) return NextResponse.json({ code: "ORDER_NOT_FOUND", error: "Pesanan tidak ditemukan." }, { status: 404, headers: noStoreHeaders() });
    ...
    if (!payment) return NextResponse.json({ code: "PAYMENT_NOT_FOUND", error: "Pembayaran untuk pesanan ini tidak ditemukan." }, { status: 404, headers: noStoreHeaders() });
    ...
    if (synchronizeProvider && paymentStatus === "pending" && payment.provider === "midtrans" && payment.provider_order_id) {
      try {
        ...
        // but the client must know why the status could not be refreshed.
        const failure = describeMidtransError(error) ?? { code: "PAYMENT_STATUS_PROVIDER_ERROR", error: "Status pembayaran belum bisa disinkronkan dengan Midtrans. Status tersimpan tetap menunggu; coba lagi.", status: 503, retryable: true };
        return NextResponse.json({ ...failure, paymentStatus }, { status: failure.status, headers: noStoreHeaders() });
      }
    }
